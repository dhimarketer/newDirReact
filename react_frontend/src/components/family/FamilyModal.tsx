...
const FamilyModal: React.FC<FamilyModalProps> = ({ isOpen, onClose, address, island }) => {
  ...
  const processFamilyMembers = (entries: PhoneBookEntry[]): FamilyMember[] => {
    ...
    // Assign roles: first 2 as parents, rest as children
    const familyMembers: FamilyMember[] = sortedEntries.map((entry, index) => ({
      entry,
      role: (index < 2 ? 'parent' : 'child') as 'parent' | 'child' | 'other',
      relationship: index < 2 ? 'Parent' : 'Child'
    }));
