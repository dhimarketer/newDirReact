#!/usr/bin/env python3
"""
2025-01-29: NEW - Execute merge for high-confidence groups (confidence >= 90)
Only processes the safest, most reliable merge candidates
"""
...
def execute_merge(candidates):
    """Execute the merge for high-confidence candidates"""
    ...
    for i, candidate in enumerate(candidates):
        try:
            ...
            print(f"📋 Processing Group {i+1}/{len(candidates)}: {candidate['name']} - {candidate['address']}")
            print(f"   Confidence: {candidate['confidence']}% | Entries: {len(entries)}")
            ...
            print(f"   ✅ Merged successfully | Primary PID: {primary_entry.pid} | Deleted: {deleted_count} entries")
...
def main():
    ...
    try:
        print("🚀 HIGH-CONFIDENCE MERGE EXECUTOR (Confidence >= 90)")
        print("=" * 60)
        ...
        # Execute merge
        merge_results = execute_merge(candidates)
